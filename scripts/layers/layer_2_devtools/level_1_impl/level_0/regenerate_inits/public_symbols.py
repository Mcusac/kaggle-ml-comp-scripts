...
Note: this file is intentionally self-contained (no `layers.*` imports).
`regenerate_inits` is designed to run even when the broader `layers/` tree has
broken imports, so importing through `layers.__init__` would defeat the point.
"""
...
def public_symbols_from_file(path: Path, *, exclude: set[str] | None = None) -> list[str]:
    try:
        src = path.read_text(encoding="utf-8")
    except OSError:
        return []
    return public_symbols_from_module(src, exclude=exclude)
