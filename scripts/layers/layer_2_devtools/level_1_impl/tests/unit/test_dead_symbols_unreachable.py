...
_MOD = _load_module_from_path(
    "_dead_symbols_unreachable",
    _SCRIPTS
    / "layers"
    / "layer_2_devtools"
    / "level_1_impl"
    / "level_2"
    / "dead_symbols_unreachable.py",
)

_compute = _MOD._compute_unreachable_payload


class TestDeadSymbolsUnreachable(unittest.TestCase):
    def test_defaults_to_cross_module_incoming_as_entrypoints(self) -> None:
    ...
    def test_respects_explicit_entrypoints(self) -> None:
        with TemporaryDirectory() as td:
            root = Path(td) / "root"
            pkg = root / "pkg"
            pkg.mkdir(parents=True)
            ...
            cfg = _MOD.DeadSymbolConfig(
                allow_symbol_ids=frozenset(),
                allow_modules=frozenset(),
                entrypoint_symbol_ids=frozenset({"pkg.a:entry"}),
            )
            payload = _compute(
                root=root,
                include_tests=False,
            ...
            dead = set(payload["dead_symbol_ids"])
            self.assertNotIn("pkg.a:helper", dead)
            self.assertNotIn("pkg.a:entry", dead)
